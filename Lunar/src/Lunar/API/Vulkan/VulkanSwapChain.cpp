...
namespace Lunar
{
	...
	{
        Arc<VulkanDevice> device = VulkanContext::GetVulkanDevice();
        device->Wait();
	...
	// Methods
    ////////////////////////////////////////////////////////////////////////////////////
	void VulkanSwapChain::Init(uint32_t width, uint32_t height, bool vsync, uint8_t framesInFlight)
	{
		if (width == 0 || height == 0)
            return;
		...
		for (uint32_t i = 0; i < imageCount; i++)
		{
			...
			if (m_Images[i])
			{
                Arc<VulkanImage> src = m_Images[i];

				// Destroy old image view
                m_Window.GetRenderer().As<VulkanRenderer>()->Free([device = device, imageView = src->m_ImageView]()
				{
                ...
                m_Images[i] = Arc<VulkanImage>::Create(m_Window.GetRenderer()->GetID(), specs, tempImages[i], imageView);
		...
		if (!m_DepthStencil)
		{
			...
			m_DepthStencil = Arc<VulkanImage>::Create(m_Window.GetRenderer()->GetID(), specs, VulkanSamplerSpecification());
		}
		else
    ...
    // Static methods
    ////////////////////////////////////////////////////////////////////////////////////
	Arc<VulkanSwapChain> VulkanSwapChain::Create(Window& window)
    {
        return Arc<VulkanSwapChain>::Create(window);
    }
    ...
    uint32_t VulkanSwapChain::AcquireNextImage()
	{
		...
		{
            Init(m_Window.GetSize().x, m_Window.GetSize().y, m_Window.GetRenderer()->GetSpecification().VSync, (uint8_t)m_Window.GetRenderer()->GetSpecification().Buffers);
		}
